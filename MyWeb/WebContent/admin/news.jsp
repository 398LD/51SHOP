	...
	<body>
							...
							<table
								...
								<tbody>
									...
									request.setCharacterEncoding("UTF-8"); 
										...
										ArrayList<En_News> list =News.QueryByPage(1);
										if(list.size()>0){
										for(int i=0;i<list.size();i++){
											En_News news=list.get(i);
											%>
											<tr>
												<td class="table-id"><%=news.getNewsID() %></td>
												<td><%=news.getNewsTitle() %></td>
												<td class="table-title"><%=news.getAdminName() %></td>
												<td class="table-title"><%=news.getNewsTime()%></td>
												<td>
												<div class="am-btn-toolbar">
													<input type="hidden" value="<%=news.getNewsID() %>">
													<input type="hidden" value="<%=news.getNewsContent()%>">
													<input type="hidden" value="<%=news.getNewsTitle()%>">
													<a onclick="edit(this);"class="am-btn am-btn-primary am-btn-xs" href="javascript:void(0);"> 
														<span></span>修改 
													</a>
													<a rel="<%=news.getNewsID() %>" onclick="del(this);" class="am-btn am-btn-danger am-btn-xs " href="javascript:void(0);"> 
														<span></span>删除
													</a>
												</div>
												</td>
											...
											out.println("<tr ><td colspan=4>没有数据</td></tr>");
		...
		<div class="am-popup" id="edit-popup">
			<div class="am-popup-inner">
				...
				<div class="am-popup-bd">
					<form action="../updNews" method="post"
...
function del(obj) {
	$('#my-confirm').modal( {
		...
		onConfirm : function(options) {
			...
			window.location.href = "../delNews?NewsID=" + delId;
		},
		onCancel : function() {
		}
	});
