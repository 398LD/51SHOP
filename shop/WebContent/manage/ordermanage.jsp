...
<!-- 循环 -->
		...
		<tr align="center">
		  ...
		  <td>
		  	<select style="height: 25px; font-size: 12px; padding: 2px;" lastval="<%=o.getStatus()%>" orderID="<%=o.getDid()%>" onchange="sck(this)" >
		  		<option value="1" <%=o.getStatus()==1?"selected":"" %> >待处理<%=OrderStatus.待处理 %></option>
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	if(pg>1){
	%>
	<a href="ordermanage.jsp?page=1&orderId=<%=orderId%>">第一页</a>　<a href="ordermanage.jsp?page=<%=pg-1%>&orderId=<%=orderId%>">上一页</a>
	<%
	}
	if(pg<maxpage){
	%>	
	　<a href="ordermanage.jsp?page=<%=pg+1%>&orderId=<%=orderId%>">下一页</a>　<a href="ordermanage.jsp?page=<%=maxpage%>&orderId=<%=orderId%>">最后一页&nbsp;</a>
	<%
	}
	%>
	</td>
...
function sck(obj){
	...
	if(obj.selectedIndex==2){
		...
		if(num!=""&&num!=null){
			...
			if(test.test(num)){
				$.get("/shop/UpdateOrderNumber?orderNumber="+num+"&id="+obj.getAttribute('orderID'),function(data){
					if(data!="ok"){
						alert("服务器发生错误");
						return;
					}else{
	...
	xhr.open("post","/shop/OrderUpdate?orderID="+obj.getAttribute('orderID')+"&status="+obj.value,false);
	xhr.send(null);
